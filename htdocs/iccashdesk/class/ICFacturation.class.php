...
class ICommFacturation {
	...
	public function sortDate($dates = NULL, $direction = 'ASC') {
		if($direction == 'ASC') {
	...
	public function ajoutArticle() {
		...
		$newcartarray = $_SESSION['icommcart'];
		...
		$_SESSION['icommcart'] = $newcartarray;

		$this->raz();
	}

	public function supprArticle($aArticle) {
		$icommcart = $_SESSION['icommcart'];

		$j=0;
		$newicommcart = array();
		foreach($icommcart as $key => $val) {
			if($icommcart[$key]['id'] != $aArticle) {
				$newicommcart[$j] = $icommcart[$key];
				$newicommcart[$j]['id'] = $j;
				$j++;
			}
		}
		unset($icommcart);
		$_SESSION['icommcart'] = $newicommcart;
	}
	...
	public function paiementLe($aPaiementLe = null) {
		if(!$aPaiementLe) {
			...
			$this->paiement_le = array();

		} else {
			$this->paiement_le[$aPaiementLe] = dol_print_date($aPaiementLe, 'dayrfc');
		}
	}
