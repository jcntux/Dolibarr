...
if ($line->fk_product > 0) 
{
	if(!class_exists('Prodoptions'))
		dol_include_once('/prodoptions/class/prodoptions.class.php');
	$product = new Prodoptions($this->db);
	$product->fetch($line->fk_product);
	
	$product->id = $line->fk_product;
	...
	if(count($prods_arbo) > 0) 
	{
		...
		{
			$product->ref=$value['ref'];
			$product->id=$value['id'];
			$str = $product->getNomUrl(0,'prodoptions').' ('.$value['nb'].') +'.price($value['price_increase']).$conf->currency;
			
			$refs[]= $form->textwithtooltip($str,$value['fullpath'],3,'','',$i,3,'');
		}
	?>
	<tr>
		<td colspan="8">
			...
			$str = implode(' ',$refs);
			print $langs->trans('InfoLineObjectWhithOptionnalProduct',$str); 
			?>
		</td>
	</tr>
	<?php 
